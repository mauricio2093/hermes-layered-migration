...
| ~~`BACKUP-DECL-006`~~ | **Closed 2026-09-18.** `backup-hermes-home.sh` declared 6 databases and 5 files in `scripts/` while the installation had 8 and 8. `shared-state.db` (Hosted Rooms) and `cron/deliveries.db` (durable cron delivery queue) arrived with Hermes 0.21.3 and were captured by nothing — the tar excludes `*.db` and neither was declared, so both would have been missing in silence. The invariant failed the backup loudly instead, which is exactly what it is for. Declarations updated consciously after verifying every addition; see [`backup-declarations.md`](backup-declarations.md). | Closed |
